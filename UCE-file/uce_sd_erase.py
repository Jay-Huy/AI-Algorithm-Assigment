...
def UCE(pipe, edit_concepts, guide_concepts, preserve_concepts, erase_scale, preserve_scale, lamb, save_dir, exp_name, uce_save_path=None):
...
def load_previous_uce_weights(unet, previous_uce_path, torch_dtype):
    if previous_uce_path is None:
        return

    state_dict = load_file(previous_uce_path)
    loaded_count = 0
        ...
        if 'attn2' in name and (name.endswith('to_v') or name.endswith('to_k')):
            ...
            if key in state_dict:
                module.weight.data.copy_(
                    state_dict[key].to(device=module.weight.device, dtype=torch_dtype)
                )
                loaded_count += 1

    if loaded_count == 0:
        print(f"Warning: no previous UCE weights loaded from {previous_uce_path}")
    else:
        print(f"Loaded {loaded_count} previous UCE weights from {previous_uce_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--previous_uce_path', help='Path to previous UCE safetensors for continual editing', type=str, default=None)
    parser.add_argument('--uce_save_path', help='Full path to save current stage UCE safetensors', type=str, default=None)
    ...
    load_previous_uce_weights(pipe.unet, args.previous_uce_path, torch_dtype)
    ...
    UCE(
        ...
        save_dir,
        exp_name,
        uce_save_path=args.uce_save_path,
    )